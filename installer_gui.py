...
class InstallerGUI:
    ...
    def __init__(self):
        ...
        # Variables
        default_path = os.path.expandvars(self.config['installation']['default_path'])
    ...
    def launch_application(self):
        ...
        try:
            ...
            # Preferir .exe si existe, sino usar Python
            exe_path = os.path.join(install_path, 'dist', 'AnalizadorBBPP_UiPath.exe')
            main_py = os.path.join(install_path, 'src', 'main.py')
            ...
            if os.path.exists(exe_path):
                ...
                python_exe = self._find_python_executable()
                if python_exe:
                    subprocess.Popen([python_exe, main_py], cwd=install_path)
                else:
                    messagebox.showwarning(
                        "Advertencia",
                        "No se encontró Python en el sistema.\n"
                        "Puedes iniciarlo manualmente desde:\n" + install_path
                    )
            else:
                messagebox.showwarning(
                    "Advertencia",
                    "No se encontró el ejecutable ni el archivo principal.\n"
                    "Puedes iniciarlo manualmente desde:\n" + install_path
                )
    ...
    def _find_python_executable(self):
        ...
        # Intentar encontrar python usando shutil.which
